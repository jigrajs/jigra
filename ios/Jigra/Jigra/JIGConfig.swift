...
@objc public class JIGConfig : NSObject {
  private static var instance: JIGConfig?
  ...
  public init(_ configText: String? = nil) {
    ...
    if let contents = configText {
      guard let configData = contents.data(using: .utf8) else {
        JIGLog.print("Unable to process config JSON string as UTF8")
        return
      }
  ...
  private func loadGlobalConfig() {
    guard let configUrl = Bundle.main.url(forResource: "jigra.config", withExtension: "json") else {
  ...
  private func parseAndSetConfig(_ data: Data) {
    do {
      ...
      JIGLog.print("Unable to parse config JSON")
