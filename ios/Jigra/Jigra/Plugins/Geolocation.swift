...
class GetLocationHandler: NSObject, CLLocationManagerDelegate {
  ...
  init(call: JIGPluginCall, options: [String:Any]) {
    self.call = call
...
@objc(JIGGeolocationPlugin)
public class JIGGeolocationPlugin : JIGPlugin {
  var locationHandler: GetLocationHandler?
  var watchLocationHandler: GetLocationHandler?
  
  @objc func getCurrentPosition(_ call: JIGPluginCall) {
  ...
  @objc func watchPosition(_ call: JIGPluginCall) {
    call.save()
  ...
  @objc func clearWatch(_ call: JIGPluginCall) {
    guard let callbackId = call.getString("id") else {
      JIGLog.print("Must supply id")
      return
    }
