...
@objc(JIGClipboardPlugin)
public class JIGClipboardPlugin : JIGPlugin {
  @objc func write(_ call: JIGPluginCall) {
    if let string = call.options["string"] as? String {
      ...
      if let data = Data(base64Encoded: getCleanData(imageBase64)) {
        ...
        JIGLog.print("Loaded image", image!.size.width, image!.size.height)
        UIPasteboard.general.image = image
      } else {
        JIGLog.print("Unable to encode image")
      }
    }
    call.success()
  }
