...
@objc(JIGSplashScreenPlugin)
public class JIGSplashScreenPlugin: JIGPlugin {
  ...
  func buildViews() {
    ...
    if image == nil {
      JIGLog.print("Unable to find splash screen image. Make sure an image called Splash exists in your assets")
    }
  ...
  func hideSplash(fadeOutDuration: Int, isLaunchSplash: Bool) {
    if isLaunchSplash, isVisible {
      JIGLog.print("SplashScreen.hideSplash: SplashScreen was automatically hidden after default timeout. " +
            "You should call `SplashScreen.hide()` as soon as your web app is loaded (or increase the timeout). " +
            "Read more at https://jigrajs.web.app/docs/apis/splash-screen#hiding-the-splash-screen")
    }
    if !isVisible { return }
    DispatchQueue.main.async {
