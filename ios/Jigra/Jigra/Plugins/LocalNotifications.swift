 ...
 */
@objc(JIGLocalNotificationsPlugin)
public class JIGLocalNotificationsPlugin : JIGPlugin {

  /**
   * Schedule a notification.
   */
  @objc func schedule(_ call: JIGPluginCall) {
    ...
    for notification in notifications {
      ...
      do {
        ...
        JIGLog.print(error.localizedDescription)
      ...
      center.add(request) { (error : Error?) in
        if let theError = error {
          JIGLog.print(theError.localizedDescription)
   ...
   */
  @objc func requestPermission(_ call: JIGPluginCall) {
   ...
   */
  @objc func cancel(_ call: JIGPluginCall) {
   ...
   */
  @objc func getPending(_ call: JIGPluginCall) {
    UNUserNotificationCenter.current().getPendingNotificationRequests(completionHandler: { (notifications) in
      JIGLog.print("num of pending notifications \(notifications.count)")
      JIGLog.print(notifications)
   ...
   */
  @objc func registerActionTypes(_ call: JIGPluginCall) {
   ...
   */
  @objc func areEnabled(_ call: JIGPluginCall) {
    let center = UNUserNotificationCenter.current()
   ...
   */
  func handleScheduledNotification(_ call: JIGPluginCall, _ schedule: [String:Any]) throws -> UNNotificationTrigger? {
  ...
  func makeAttachmentUrl(_ path: String) -> URL? {
    let file = JIGFileManager.get(path: path)
    return file?.url
  }
