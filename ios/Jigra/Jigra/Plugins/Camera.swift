...
@objc(JIGCameraPlugin)
public class JIGCameraPlugin : JIGPlugin, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIPopoverPresentationControllerDelegate {
  let DEFAULT_SOURCE = CameraSource.prompt
  let DEFAULT_DIRECTION = CameraDirection.rear

  var imagePicker: UIImagePickerController?
  var call: JIGPluginCall?

  var imageCounter = 0

  var settings = CameraSettings()
  
  @objc func getPhoto(_ call: JIGPluginCall) {
    self.call = call
  ...
  func getSettings(_ call: JIGPluginCall) -> CameraSettings {
    var settings = CameraSettings()
  ...
  func doShow(call: JIGPluginCall, settings: CameraSettings) {
  ...
  func showPrompt(_ call: JIGPluginCall) {
  ...
  func showCamera(_ call: JIGPluginCall) {
  ...
  func showPhotos(_ call: JIGPluginCall) {
  ...
  public func imagePickerController(_ picker: UIImagePickerController,
    ...
    if settings.resultType == CameraResultType.base64.rawValue {
      ...
      guard let webPath = JIGFileManager.getPortablePath(host: bridge.getLocalUrl(), uri: URL(string: path)) else {
