...
public class JIGBridgeViewController: UIViewController, JIGBridgeDelegate, WKScriptMessageHandler, WKUIDelegate, WKNavigationDelegate {
  ...
  // Construct the Jigra runtime
  public var bridge: JIGBridge?
  private var handler: JIGAssetHandler?
  ...
  override public func loadView() {
    ...
    let jigConfig = JIGConfig(self.config)

    HTTPCookieStorage.shared.cookieAcceptPolicy = HTTPCookie.AcceptPolicy.always
    let webViewConfiguration = WKWebViewConfiguration()
    self.handler = JIGAssetHandler()
    self.handler!.setAssetPath(startPath)
    var specifiedScheme = JIGBridge.JIG_DEFAULT_SCHEME
    let configScheme = jigConfig.getString("server.iosScheme") ?? JIGBridge.JIG_DEFAULT_SCHEME
    ...
    if let appendUserAgent = (jigConfig.getValue("ios.appendUserAgent") as? String) ?? (jigConfig.getValue("appendUserAgent") as? String) {
    ...
    if let contentInset = (jigConfig.getValue("ios.contentInset") as? String),
    ...
    if let allowsLinkPreview = (jigConfig.getValue("ios.allowsLinkPreview") as? Bool) {
    ...
    bridge = JIGBridge(self, o, jigConfig, specifiedScheme)
  ...
  private func getStartPath() -> String? {
    ...
    if !isDeployDisabled() && !isNewBinary() {
      ...
      if (persistedPath != nil && !persistedPath!.isEmpty) {
        ...
        let snapshots = (cordovaDataDirectory as NSString).appendingPathComponent("family_built_snapshots")
  ...
  func printLoadError() {
    ...
    JIGLog.print("⚡️  ERROR: Unable to load \(fullStartPath.relativePath)/index.html")
  ...
  func loadWebView() {
    ...
    if bridge!.isDevMode() && bridge!.config.getString("server.url") != nil {
      let toastPlugin = bridge!.getOrLoadPlugin(pluginName: "Toast") as? JIGToastPlugin
      toastPlugin!.showToast(vc: self, text: "Using app server \(hostname!)", duration: 3500)
    }

    JIGLog.print("⚡️  Loading app at \(hostname!)...")
  ...
  public func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
    NotificationCenter.default.post(name: Notification.Name(JIGNotifications.DecidePolicyForNavigationAction.name()), object: navigationAction)
  ...
  public func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
    ...
    JIGLog.print("⚡️  WebView failed to load")
    JIGLog.print("⚡️  Error: " + error.localizedDescription)
  }

  public func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
    JIGLog.print("⚡️  WebView failed provisional navigation")
    JIGLog.print("⚡️  Error: " + error.localizedDescription)
  }
