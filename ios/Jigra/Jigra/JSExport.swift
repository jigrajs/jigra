...
public class JSExport {
  ...
  public static func exportJigraGlobalJS(userContentController: WKUserContentController, isDebug: Bool, localUrl: String) throws {
    let data = "window.Jigra = { DEBUG: \(isDebug), Plugins: {} }; window.WEBVIEW_SERVER_URL = '\(localUrl)';"
    let userScript = WKUserScript(source: data, injectionTime: .atDocumentStart, forMainFrameOnly: true)
    userContentController.addUserScript(userScript)
  }
  
  public static func exportJigraJS(userContentController: WKUserContentController) throws {
   ...
   */
  public static func exportJS(userContentController: WKUserContentController, pluginClassName: String, pluginType: JIGPlugin.Type) {
    var lines = [String]()
    
    lines.append("""
      (function(w) {
      w.Jigra = w.Jigra || {};
      w.Jigra.Plugins = w.Jigra.Plugins || {};
      var a = w.Jigra; var p = a.Plugins;
    ...
    let methods = bridgeType.pluginMethods() as! [JIGPluginMethod]
  ...
  private static func generateMethod(pluginClassName: String, method: JIGPluginMethod) -> String {
    ...
    if returnType == JIGPluginReturnNone {
      ...
      // ...using a callback
      lines.append("""
        return w.Jigra.nativeCallback('\(pluginClassName)', '\(methodName)', \(argObjectString), \(CALLBACK_PARAM));
        """)
    } else {
