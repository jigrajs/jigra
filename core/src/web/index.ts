...
export class WebPlugin {
  ...
  requestPermissions(): Promise<PermissionsRequestResult> {
    if (Jigra.isNative) {
      return Jigra.nativePromise(this.config.name, 'requestPermissions', {});
    } else {
...
const shouldMergeWebPlugin = (plugin: WebPlugin) => {
  return plugin.config.platforms && plugin.config.platforms.indexOf(Jigra.platform) >= 0;
};
 ...
 * @param knownPlugins the Jigra.Plugins global registry.
 */
