...
public class MockCordovaWebViewImpl implements CordovaWebView {
  ...
  private JigraCordovaCookieManager cookieManager;
  ...
  public void init(CordovaInterface cordova, List<PluginEntry> pluginEntries, CordovaPreferences preferences, WebView webView) {
    ...
    nativeToJsMessageQueue.addBridgeMode(new JigraEvalBridgeMode(webView, this.cordova));
    nativeToJsMessageQueue.setBridgeMode(0);
    this.cookieManager = new JigraCordovaCookieManager(webView);
    this.pluginManager.init();
  }

  public static class JigraEvalBridgeMode extends NativeToJsMessageQueue.BridgeMode {
    private final WebView webView;
    private final CordovaInterface cordova;

    public JigraEvalBridgeMode(WebView webView, CordovaInterface cordova) {
      this.webView = webView;
      this.cordova = cordova;
    }
  ...
  public void triggerDocumentEvent(final String eventName) {
    eval("window.Jigra.triggerEvent('" + eventName + "', 'document');", new ValueCallback<String>() {
      @Override
