...
public class WebViewLocalServer {
  ...
  private final static String jigraFileStart = Bridge.JIGRA_FILE_START;
  private final static String jigraContentStart = Bridge.JIGRA_CONTENT_START;
  ...
  private boolean isLocalFile(Uri uri) {
    String path = uri.getPath();
    if (path.startsWith(jigraContentStart) || path.startsWith(jigraFileStart)) {
      return true;
    }
    return false;
  }
  ...
  private void createHostingDetails() {
    ...
    PathHandler handler = new PathHandler() {
      ...
      public InputStream handle(Uri url) {
        ...
        try {
          if (path.startsWith(jigraContentStart)) {
            stream = protocolHandler.openContentUrl(url);
          } else if (path.startsWith(jigraFileStart) || !isAsset) {
            if (!path.startsWith(jigraFileStart)) {
    ...
    for (String authority: authorities) {
      registerUriForScheme(Bridge.JIGRA_HTTP_SCHEME, handler, authority);
      registerUriForScheme(Bridge.JIGRA_HTTPS_SCHEME, handler, authority);

      String customScheme = this.bridge.getScheme();
      if (!customScheme.equals(Bridge.JIGRA_HTTP_SCHEME) && !customScheme.equals(Bridge.JIGRA_HTTPS_SCHEME)) {
