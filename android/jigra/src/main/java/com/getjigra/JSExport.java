...
public class JSExport {
  ...
  public static String getGlobalJS(Context context, boolean isDebug) {
    return "window.Jigra = { DEBUG: " + isDebug + " };";
  }
  ...
  public static String getCoreJS(Context context) throws JSExportException {
    try {
      ...
      throw new JSExportException("Unable to load native-bridge.js. Jigra will not function!", ex);
    }
  }
  ...
  public static String getPluginJS(Collection<PluginHandle> plugins) {
    ...
    lines.add("// Begin: Jigra Plugin JS");
    ...
    for(PluginHandle plugin : plugins) {
      lines.add("(function(w) {\n" +
          ...
          "  return w.Jigra.addListener('" + plugin.getId() + "', eventName, callback);\n" +
          "}");
  ...
  private static String generateMethodJS(PluginHandle plugin, PluginMethodHandle method) {
    ...
    switch(returnType) {
      case PluginMethod.RETURN_NONE:
        lines.add("return w.Jigra.nativeCallback('" + plugin.getId() + "', '" + method.getName() + "', " + CATCHALL_OPTIONS_PARAM + ")");
        break;
      case PluginMethod.RETURN_PROMISE:
        lines.add("return w.Jigra.nativePromise('" + plugin.getId() + "', '" + method.getName() + "', " + CATCHALL_OPTIONS_PARAM + ")");
        break;
      case PluginMethod.RETURN_CALLBACK:
        lines.add("return w.Jigra.nativeCallback('" + plugin.getId() + "', '" +
