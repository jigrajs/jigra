...
import com.getjigra.android.BuildConfig;
import com.getjigra.plugin.Accessibility;
import com.getjigra.plugin.App;
import com.getjigra.plugin.Browser;
import com.getjigra.plugin.Camera;
import com.getjigra.plugin.Clipboard;
import com.getjigra.plugin.Device;
import com.getjigra.plugin.Filesystem;
import com.getjigra.plugin.Geolocation;
import com.getjigra.plugin.Haptics;
import com.getjigra.plugin.Keyboard;
import com.getjigra.plugin.LocalNotifications;
import com.getjigra.plugin.Modals;
import com.getjigra.plugin.Network;
import com.getjigra.plugin.Permissions;
import com.getjigra.plugin.Photos;
import com.getjigra.plugin.PushNotifications;
import com.getjigra.plugin.Share;
import com.getjigra.plugin.SplashScreen;
import com.getjigra.plugin.StatusBar;
import com.getjigra.plugin.Storage;
import com.getjigra.plugin.background.BackgroundTask;
import com.getjigra.ui.Toast;
import com.getjigra.util.HostMask;
...
/**
 * The Bridge class is the main engine of Jigra. It manages
 ...
 * <a href="https://github.com/jigrajs/jigra/blob/2.x/android/jigra/src/main/java/com/getjigra/BridgeActivity.java">
 *   BridgeActivity.java</a>
 */
public class Bridge {

  private static final String PREFS_NAME = "JigraSettings";
  private static final String BUNDLE_LAST_PLUGIN_ID_KEY = "jigraLastActivityPluginId";
  private static final String BUNDLE_LAST_PLUGIN_CALL_METHOD_NAME_KEY = "jigraLastActivityPluginMethod";
  private static final String BUNDLE_PLUGIN_CALL_OPTIONS_SAVED_KEY = "jigraLastPluginCallOptions";
  private static final String BUNDLE_PLUGIN_CALL_BUNDLE_KEY = "jigraLastPluginCallBundle";
  ...
  public static final String JIGRA_HTTP_SCHEME = "http";
  ...
  private final HandlerThread handlerThread = new HandlerThread("JigraPlugins");
  ...
  public Bridge(Activity context, WebView webView, List<Class<? extends Plugin>> initialPlugins, CordovaInterfaceImpl cordovaInterface, PluginManager pluginManager, CordovaPreferences preferences, JSONObject config) {
    ...
    this.config = new JigConfig(getActivity().getAssets(), config);
  ...
  private void loadWebView() {
    ...
    if (appUrlConfig != null) {
      ...
      if (!scheme.equals(Bridge.JIGRA_HTTP_SCHEME) && !scheme.equals(JIGRA_HTTPS_SCHEME)) {
        appUrl += "/";
      }
    }
    ...
    if (!isDeployDisabled() && !isNewBinary()) {
      ...
      String path = prefs.getString(com.getjigra.plugin.WebView.JIG_SERVER_PATH, null);
  ...
  private boolean isNewBinary() {
    ...
    if (!versionCode.equals(lastVersionCode) || !versionName.equals(lastVersionName)) {
      ...
      editor.putString(com.getjigra.plugin.WebView.JIG_SERVER_PATH, "");
      editor.apply();
      return true;
    }
    return false;
  ...
  public void handleAppUrlLoadError(Exception ex) {
    if (ex instanceof SocketTimeoutException) {
      ...
      Logger.error("Unable to load app. Ensure the server is running at " + appUrl + ", or modify the " +
          "appUrl setting in jigra.config.json (make sure to npx jig copy after to commit changes).", ex);
    }
  }
  ...
  /**
   * Get the core WebView under Jigra's control
   * @return
   */
  ...
  public String getScheme() {
      return this.config.getString("server.androidScheme", JIGRA_HTTP_SCHEME);
  }

  public JigConfig getConfig() {
    return this.config;
  }
  ...
  private void registerAllPlugins() {
    ...
    this.registerPlugin(com.getjigra.plugin.Toast.class);
    this.registerPlugin(com.getjigra.plugin.WebView.class);
  ...
  public void logToJs(final String message, final String level) {
    eval("window.Jigra.logJs(\"" + message + "\", \"" + level + "\")", null);
  }
  ...
  public void triggerJSEvent(final String eventName, final String target) {
    eval("window.Jigra.triggerEvent(\"" + eventName + "\", \"" + target + "\")", new ValueCallback<String>() {
      @Override
  ...
  public void triggerJSEvent(final String eventName, final String target, final String data) {
    eval("window.Jigra.triggerEvent(\"" + eventName + "\", \"" + target + "\", " + data + ")", new ValueCallback<String>() {
      @Override
      public void onReceiveValue(String s) {
      }
    });
   ...
   * to ensure that Jigra's JS and the JS for all the plugins is loaded each time.
   */
  private JSInjector getJSInjector() {
    try {
      ...
      Logger.error("Unable to export Jigra JS. App will not function!", ex);
    }
    return null;
  }
  ...
  public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
    ...
    if (plugin == null) {
      Logger.debug("Unable to find a Jigra plugin to handle permission requestCode, trying Cordova plugins " + requestCode);
      try {
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (plugin == null || plugin.getInstance() == null) {
      Logger.debug("Unable to find a Jigra plugin to handle requestCode, trying Cordova plugins " + requestCode);
