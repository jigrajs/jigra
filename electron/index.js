...
const configJigra = async function(mainWindow) {
  let jigConfigJson = JSON.parse(fs.readFileSync(`./jigra.config.json`, 'utf-8'));
  const appendUserAgent = jigConfigJson.electron && jigConfigJson.electron.appendUserAgent ? jigConfigJson.electron.appendUserAgent : jigConfigJson.appendUserAgent;
  if (appendUserAgent) {
    mainWindow.webContents.setUserAgent(mainWindow.webContents.getUserAgent() + " " + appendUserAgent);
  }
  const overrideUserAgent = jigConfigJson.electron && jigConfigJson.electron.overrideUserAgent ? jigConfigJson.electron.overrideUserAgent : jigConfigJson.overrideUserAgent;
  if (overrideUserAgent) {
    mainWindow.webContents.setUserAgent(overrideUserAgent);
  }
}

class JigraSplashScreen {
  ...
  constructor(mainWindow, splashOptions) {
    ...
    try {
      ...
      this.splashOptions = Object.assign(
        this.splashOptions,
        jigConfigJson.plugins.SplashScreen || {}
      );
    } catch (e) {
      console.error(e.message);
    }

    ipcMain.on('showJigraSplashScreen', (event, options) => {
      this.show();
      if (options) {
        if (options.autoHide) {
    ...
    ipcMain.on('hideJigraSplashScreen', (event, options) => {
      this.hide();
    });
  }
