...
export async function copyCordovaJS(config: Config, platform: string) {
  const cordovaPath = resolveNode(config, '@jigra/core', 'cordova.js');
  if (!cordovaPath) {
    logFatal(`Unable to find node_modules/@jigra/core/cordova.js. Are you sure`,
...
export function getIncompatibleCordovaPlugins(platform: string) {
  let pluginList = ['cordova-plugin-splashscreen', 'cordova-plugin-family-webview', 'cordova-plugin-crosswalk-webview',
  'cordova-plugin-wkwebview-engine', 'cordova-plugin-console', 'cordova-plugin-music-controls',
  'cordova-plugin-add-swift-support', 'cordova-plugin-family-keyboard', 'cordova-plugin-braintree',
  '@family-enterprise/filesystem', '@family-enterprise/keyboard', '@family-enterprise/splashscreen', 'cordova-support-google-services'];
  if (platform === 'ios') {
    pluginList.push('cordova-plugin-googlemaps', 'cordova-plugin-statusbar', '@family-enterprise/statusbar');
  }
  if (platform === 'android') {
...
export async function getCordovaPreferences(config: Config) {
  ...
  if (config.app.extConfig && config.app.extConfig.cordova && config.app.extConfig.cordova.preferences && cordova.preferences) {
    const answer = await inquirer.prompt({
      type: 'confirm',
      name: 'confirm',
      message: 'jigra.config.json already contains cordova preferences. Overwrite with values from config.xml?'
    });
    if (!answer.confirm) {
...
<manifest package="jigra.android.plugins"
