...
export async function checkWebDir(config: Config): Promise<string | null> {
  ...
  if (!await existsAsync(config.app.webDirAbs)) {
    return `Jigra could not find the web assets directory "${config.app.webDirAbs}".
    Please create it and make sure it has an index.html file. You can change
    the path of this directory in jigra.config.json (webDir option).
    You may need to compile the web assets for your app (typically 'npm run build').
    More info: https://jigrajs.web.app/docs/basics/building-your-app`;
  }
...
export async function checkPackage(_config: Config): Promise<string | null> {
  if (!await existsAsync('package.json')) {
    return `Jigra needs to run at the root of an npm package.
    Make sure you have a "package.json" in the directory where you run jigra.
...
export async function checkAppConfig(config: Config): Promise<string | null> {
  if (!config.app.appId) {
    return 'Missing appId for new platform. Please add it in jigra.config.json or run npx jig init.';
  }
  if (!config.app.appName) {
    return 'Missing appName for new platform. Please add it in jigra.config.json or run npx jig init.';
  }
...
export async function printNextSteps(config: Config, appDir: string) {
  log('\n');
  log(`${chalk.bold(`${_e('🎉', '*')}   Your Jigra project is ready to go!  ${_e('🎉', '*')}`)}\n`);
  if (appDir !== '') {
    log(`Next steps:`);
    log('');
  ...
  log(`Add platforms using "npx jig add":\n`);
  log(`  npx jig add android`);
  log(`  npx jig add ios`);
  log(`  npx jig add electron`);
  log('');
  log(`Follow the Developer Workflow guide to get building:\n${chalk.bold(`https://jigrajs.web.app/docs/basics/workflow`)}\n`);
}

export async function getCoreVersion(config: Config): Promise<string> {
  const corePackagePath = resolveNode(config, '@jigra/core', 'package.json');
  if (!corePackagePath) {
    logFatal('Unable to find node_modules/@jigra/core/package.json. Are you sure',
      '@jigra/core is installed? This file is currently required for Jigra to function.');
  }
...
export async function getCLIVersion(config: Config): Promise<string> {
  const cliPackagePath = resolveNode(config, '@jigra/cli', 'package.json');
  if (!cliPackagePath) {
    logFatal('Unable to find node_modules/@jigra/cli/package.json. Are you sure',
      '@jigra/cli is installed? This file is currently required for Jigra to function.');
  }
...
export async function getPlatformVersion(config: Config, platform: string): Promise<string> {
  const platformPackagePath = resolveNode(config, `@jigra/${platform}`, 'package.json');
  if (!platformPackagePath) {
    logFatal(`Unable to find node_modules/@jigra/${platform}/package.json. Are you sure`,
      `@jigra/${platform} is installed? This file is currently required for Jigra to function.`);
  }
...
export async function checkPlatformVersions(config: Config, platform: string) {
  ...
  if (semver.diff(coreVersion, platformVersion) === 'minor' || semver.diff(coreVersion, platformVersion) === 'major') {
    log('\n');
    logWarn(`Your @jigra/core version doesn't match your @jigra/${platform} version`);
    log(`Consider updating to matching version ${chalk`{bold npm install @jigra/core@${platformVersion}}`}`);
  }
}

export function resolvePlatform(config: Config, platform: string): string | null {
  if (platform[0] !== '@') {
    const core = resolveNode(config, `@jigra/${platform}`);

    if (core) {
      return core;
    }

    const community = resolveNode(config, `@jigra-community/${platform}`);
...
export async function checkNPMVersion() {
  ...
  if (semver.gt(minVersion, version)) {
    return `Jigra CLI requires at least NPM ${minVersion}`;
  }
  return null;
}
...
export function electronWarning() {
  ...
  log(`\nPlease use the Jigra Community Electron Platform: https://github.com/jigra-community/electron\n`);
}
