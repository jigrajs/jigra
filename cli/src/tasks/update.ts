...
export async function updateCommand(config: Config, selectedPlatformName: string, deployment: boolean) {
  if (selectedPlatformName && !config.isValidPlatform(selectedPlatformName)) {
    ...
    if (platformFolder) {
      const result = await runPlatformHook(`cd "${platformFolder}" && ${await hasYarn(config) ? 'yarn' : 'npm'} run jigra:update`);
      log(result);
    } else {
    ...
    if (platforms.length === 0) {
      logInfo(`There are no platforms to update yet. Create one with "jigra create".`);
      return;
    }
    try {
      await check(
