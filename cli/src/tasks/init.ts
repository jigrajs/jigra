...
export async function initCommand(config: Config, name: string, id: string, webDir: string, client: string) {
  ...
  try {
    ...
    await runTask(`Initializing Jigra project in ${chalk.blue(config.app.rootDir)}`, async () => {
    ...
    log('Usage: npx jig init appName appId\n');
    log('Example: npx jig init "My App" "com.example.myapp"\n');
    logFatal(e);
  }
}
