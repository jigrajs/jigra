...
export async function createCommand(config: Config, dir: string, name: string, id: string, client: string) {
  ...
  try {
    ...
    log(chalk`\n{bold ${_e('⚡️', '*')}   Welcome to Jigra (CLI v${cliVersion}) ${_e('⚡️', '*')}}\n`);
    ...
    await runTask(chalk`Installing dependencies`, () => {
      return installDeps(appDir, ['@jigra/cli', '@jigra/core'], config);
    });
    // Copy web and jigra to web assets
    ...
    if (typeof e === 'string') {
      log('Usage: npx @jigra/cli create appDir appName appId npmClient?');
      log('Example: npx @jigra/cli create my-app "My App" "com.example.myapp"');
    }
