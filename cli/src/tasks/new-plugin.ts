...
export async function newPlugin(config: Config) {
  log(`${emoji('✏️', '*')}  Creating new Jigra plugin`);
  ...
  const answers: NewPluginAnswers = await inquirer.prompt([
    {
      type: 'input',
      name: 'name',
      message: 'Plugin npm name (kebab-case. ex: jigra-plugin-example):',
  ...
  if (answers.confirm) {
    ...
    logInfo(`Your Jigra plugin was created at ${pluginPath}`);
...
function generatePackageJSON(answers: NewPluginAnswers, cliVersion: string) {
  return {
    ...
    'devDependencies': {
      ...
      '@familyjs/swiftlint-config': '^1.1.2',
      '@jigra/android': `^${cliVersion}`,
      '@jigra/core': `^${cliVersion}`,
      '@jigra/ios': `^${cliVersion}`,
