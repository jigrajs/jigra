...
export async function doctorCommand(config: Config, selectedPlatform: string): Promise<void> {
  log(`${_e('💊', '')}   ${chalk.bold('Jigra Doctor')}  ${_e('💊', '')} \n`);
...
export async function doctorCore(config: Config) {
  let cliVersion = await runCommand(`npm info @jigra/cli version`);
  let coreVersion = await runCommand(`npm info @jigra/core version`);
  let androidVersion = await runCommand(`npm info @jigra/android version`);
  let electronVersion = await runCommand(`npm info @jigra/android version`);
  let iosVersion = await runCommand(`npm info @jigra/ios version`);

  log(`${chalk.bold.blue('Latest Dependencies:')}\n`);
  log(`  ${chalk.bold('@jigra/cli:')}`, cliVersion.trim());
  log(`  ${chalk.bold('@jigra/core:')}`, coreVersion.trim());
  log(`  ${chalk.bold('@jigra/android:')}`, androidVersion.trim());
  log(`  ${chalk.bold('@jigra/electron:')}`, electronVersion.trim());
  log(`  ${chalk.bold('@jigra/ios:')}`, iosVersion.trim());
...
async function printInstalledPackages(config: Config) {
  const packageNames = ['@jigra/cli', '@jigra/core', '@jigra/android', '@jigra/ios'];
  await Promise.all(packageNames.map(async packageName => {
    const packagePath = resolveNode(config, packageName, 'package.json');
    await printPackageVersion(packageName, packagePath);
  }));
  const packagePath = resolveNodeFrom(config.electron.platformDir, '@jigra/electron');
  await printPackageVersion('@jigra/electron', packagePath ? join(packagePath, 'package.json') : packagePath);
}
