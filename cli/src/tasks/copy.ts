...
export async function copyCommand(config: Config, selectedPlatformName: string) {
  if (selectedPlatformName && !config.isValidPlatform(selectedPlatformName)) {
    ...
    if (platformFolder) {
      const result = await runPlatformHook(`cd "${platformFolder}" && ${await hasYarn(config) ? 'yarn' : 'npm'} run jigra:copy`);
      log(result);
    } else {
    ...
    if (platforms.length === 0) {
      logInfo(`There are no platforms to copy yet. Create one with \`jigra create\`.`);
      return;
    }
    try {
...
export async function copy(config: Config, platformName: string) {
  await runTask(chalk`{green {bold copy}}`, async () => {
    ...
    if (platformName === config.ios.name) {
      ...
      await copyJigraConfig(config, join(config.ios.platformDir, config.ios.nativeProjectName, config.ios.nativeProjectName));
      ...
      await copyJigraConfig(config, join(config.android.platformDir, 'app/src/main/assets'));
      const cordovaPlugins = await getCordovaPlugins(config, platformName);
      await handleCordovaPluginsJS(cordovaPlugins, config, platformName);
      ...
      await copyJigraConfig(config, config.electron.platformDir);
      electronWarning();
    } else {
...
async function copyNativeBridge(config: Config, nativeAbsDir: string) {
  let bridgePath = resolveNode(config, '@jigra/core', 'native-bridge.js');
  if (!bridgePath) {
    logFatal(`Unable to find node_modules/@jigra/core/native-bridge.js. Are you sure`,
...
async function copyJigraConfig(config: Config, nativeAbsDir: string) {
  ...
  await runTask('Copying jigra.config.json', async () => {
    return fsCopy(configPath, join(nativeAbsDir, 'jigra.config.json'));
  });
}
