...
export async function addCommand(config: Config, selectedPlatformName: string) {
  if (selectedPlatformName && !config.isValidPlatform(selectedPlatformName)) {
    ...
    if (platformFolder) {
      const result = await runPlatformHook(`cd "${platformFolder}" && ${await hasYarn(config) ? 'yarn' : 'npm'} run jigra:add`);
      log(result);
    } else {
    ...
    try {
      ...
      await generateJigraConfig(config);
      ...
      if (platformName === config.ios.name || platformName === config.android.name) {
        log(chalk`\nNow you can run {green {bold npx jig open ${platformName}}} to launch ${platformName === config.ios.name ? 'Xcode' : 'Android Studio'}`);
      }
    } catch (e) {
      logFatal(e);
    }
...
export async function generateJigraConfig(config: Config) {
...
function webWarning() {
  ...
  log(`\nIn Jigra, the 'web' platform is just your web app!`);
  log(`For example, if you have a React or Angular project, the 'web' platform is that project.`);
  log(`To add Jigra functionality to your web app, follow the Web Getting Started Guide: https://jigrajs.web.app/docs/web`);
}
