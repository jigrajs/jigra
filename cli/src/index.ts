...
export function run(process: NodeJS.Process, cliBinDir: string) {
  ...
  program
    ...
    .description('Creates a new Jigra project')
  ...
  program
    ...
    .description('Initializes a new Jigra project in the current directory')
  ...
  program
    .command('serve')
    .description('Serves a Jigra Progressive Web App in the browser')
    .action(() => {
  ...
  program
    ...
    .description('start a new Jigra plugin')
    .action(() => {
  ...
  if (program.rawArgs.length < 3) {
    console.log(`\n  ${_e('⚡️', '--')}  ${chalk.bold('Jigra - Cross-Platform apps with JavaScript and the Web')}  ${_e('⚡️', '--')}`);
    program.help();
  }
}
