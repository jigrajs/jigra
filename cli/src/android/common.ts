...
export async function editProjectSettingsAndroid(config: Config) {
  ...
  await copyAsync(
    resolve(config.app.rootDir, config.android.platformDir, 'app/src/main/java/com/getjigra/myapp/MainActivity.java'),
    resolve(newJavaPath, 'MainActivity.java')
  );

  if (appId.split('.')[1] !== 'getjigra') {
    await removeAsync(resolve(config.app.rootDir, config.android.platformDir, 'app/src/main/java/com/getjigra'));
  }
  ...
  stringsContent = stringsContent.replace(/com.getjigra.myapp/g, appId);
