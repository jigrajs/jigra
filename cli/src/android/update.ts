...
export async function updateAndroid(config: Config) {
  ...
  await installGradlePlugins(config, jigraPlugins, cordovaPlugins);
...
export async function installGradlePlugins(config: Config, jigraPlugins: Plugin[], cordovaPlugins: Plugin[]) {
  const jigraAndroidPath = resolveNode(config, '@jigra/android', 'jigra');
  if (!jigraAndroidPath) {
    logFatal(`Unable to find node_modules/@jigra/android/jigra. Are you sure`,
      `@jigra/android is installed? This file is currently required for Jigra to function.`);
    return;
  }
...
`;
  ...
  await writeFileAsync(join(settingsPath, 'jigra.settings.gradle'), settingsLines);
  await writeFileAsync(join(dependencyPath, 'jigra.build.gradle'), dependencyLines);
}
...
export async function handleCordovaPluginsGradle(config: Config,  cordovaPlugins: Plugin[]) {
  ...
  const cordovaVariables = `// DO NOT EDIT THIS FILE! IT IS GENERATED EACH TIME "jigra update" IS RUN
ext {
