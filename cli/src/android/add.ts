...
export async function addAndroid(config: Config) {
  ...
  await runTask(`Installing android dependencies`, async (info: TaskInfoProvider) => {
    if (resolveNode(config, '@jigra/android')) {
      info('Skipping: already installed');
      return;
    }
    const cliVersion = await getCLIVersion(config);
    return installDeps(config.app.rootDir, [`@jigra/android@${cliVersion}`], config);
  });
...
sdk.dir=${defaultAndroidPath}
  `;
    ...
    try {
      ...
      console.error('Unable to infer default Android SDK settings. This is fine, just run npx jig open android and import and sync gradle manually');
    }
  }
}
