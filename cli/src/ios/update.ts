...
export async function updatePodfile(config: Config, plugins: Plugin[], deployment: boolean) {
  ...
  podfileContent = podfileContent.replace(/(Automatic Jigra Pod dependencies, do not delete)[\s\S]*(# Do not delete)/, '$1' + dependenciesContent + '\n  $2');
...
export function generatePodFile(config: Config, plugins: Plugin[]) {
  const jigraiOSPath = resolveNode(config, '@jigra/ios');
  if (!jigraiOSPath) {
    logFatal(`Unable to find node_modules/@jigra/ios. Are you sure`,
      `@jigra/ios is installed? This file is currently required for Jigra to function.`);
    return;
  }

  const podfilePath = join(config.app.rootDir, 'ios', 'App');
  const relativeJigraiOSPath = convertToUnixPath(relative(podfilePath, realpathSync(jigraiOSPath)));

  const jigraPlugins = plugins.filter(p => getPluginType(p, platform) === PluginType.Core);
  const pods = jigraPlugins
  ...
  if (noPodPlugins.length > 0) {
    pods.push(`pod 'CordovaPlugins', :path => '../jigra-cordova-ios-plugins'`);
  }
  const podPlugins = cordovaPlugins.filter((el) => !noPodPlugins.includes(el));
  if (podPlugins.length > 0) {
    pods.push(`pod 'CordovaPluginsStatic', :path => '../jigra-cordova-ios-plugins'`);
  }
  const resourcesPlugins = cordovaPlugins.filter(filterResources);
  if (resourcesPlugins.length > 0) {
    pods.push(`pod 'CordovaPluginsResources', :path => '../jigra-cordova-ios-plugins'`);
  }
    return `
  pod 'Jigra', :path => '${relativeJigraiOSPath}'
  pod 'JigraCordova', :path => '${relativeJigraiOSPath}'
  ${pods.join('\n  ')}`;
}
...
async function generateCordovaPodspec(cordovaPlugins: Plugin[], config: Config, isStatic: boolean) {
  ...
  Pod::Spec.new do |s|
    ...
    s.source = { :git => 'https://github.com/username/does-not-exist.git', :tag => '${config.cli.package.version}' }
    ...
    s.dependency 'JigraCordova'${getLinkerFlags(config)}
